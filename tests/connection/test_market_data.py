...
def mock_ws_connect(mock_ws):
    ...
    with patch("src.connection.market_data.websockets.connect") as mock:
        cm = AsyncMock()
...
async def test_handle_ticker_calls_sync_callback():
    ...
    with patch("src.connection.market_data.Ticker") as mock_ticker_cls:
...
async def test_handle_candle_calls_sync_callback():
    ...
    with patch("src.connection.market_data.Candle") as mock_candle_cls:
...
async def test_connect_increments_reconnect_count_on_error():
    ...
    with (
        patch("src.connection.market_data.websockets.connect", side_effect=OSError("연결 실패")),
